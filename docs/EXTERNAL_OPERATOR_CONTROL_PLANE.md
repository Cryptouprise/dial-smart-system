...
| MCP | The package now exposes the six shared R0 observer tools, including the Elite Solar brief and morning pulse. It submits only to a dedicated `mcp-observer` adapter backed by MCP-specific identity binding and durable receipts; it no longer treats the legacy API gateway as an MCP surface. The Edge entry point is compile-time locked, unprovisioned, and not live. |
...
5. For MCP, mint one short-lived, revocable, read-only `dsk_live_` API key bound to the test organization and owner/admin user **through an `mcp` installation and principal**, never a Zapier binding. Limit its scopes to `system:read` and `campaigns:read`; it must be supplied only as an HTTP Bearer credential to the dedicated `mcp-observer` endpoint. Every action body carries its exact-UTC `source_occurred_at`, and retry retains the same payload-bound request identity rather than receiving a new server timestamp.
