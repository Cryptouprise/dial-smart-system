...
import { DemoLeadRecovery } from './DemoLeadRecovery';
...
export const DemoLegalVisionWithVSL = (props: DemoLegalVisionWithVSLProps) => {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      observer?.disconnect();
      if (video && (video as any).__callBossVslCleanup) (video as any).__callBossVslCleanup();
    };
  }, [props.sessionId, showRecovery]);
  ...
  if (showRecovery) {
    return (
      <DemoLeadRecovery
        ...
        sessionId={props.sessionId}
        averageClientValue={props.legalInboundConfig.averageClientValue}
        onBack={() => {
        ...
        onContinue={() => {
          ...
          requestAnimationFrame(() => {
            const button = Array.from(document.querySelectorAll<HTMLButtonElement>('button'))
              .find((candidate) => candidate.textContent?.includes('Show Me Recovery'));
            button?.click();
          });
        }}
      />
