...
const PROVIDER_READINESS: readonly ProviderReadiness[] = [
  ...
  {
    name: 'Telnyx',
    focus: 'Telephony, phone-number, and AI-assistant provider',
    requiredEvidence: [
  ...
  {
    name: 'GoHighLevel',
    focus: 'CRM shadow source—not a contact authority',
    requiredEvidence: [
...
const BINDING_STEPS = [
  'A service-side owner stores the credential in the approved secret store; it never enters this browser or a client-side database row.',
  'The binding service verifies the exact account and resource ownership, then records non-secret resource fingerprints against one organization.',
  'The service produces a read-only candidate bundle: provider, tenant, exact resource IDs, webhook identity, secret reference, and evidence fingerprint.',
  'A human reviewer certifies the candidate bundle alongside consent, DNC, jurisdiction, policy, balance, and global-stop evidence.',
  'Only a later, separately reviewed server release may use the certified bundle for an owned-phone canary or a bounded human cohort.',
] as const;
  ...
  <div className="space-y-6" data-testid="provider-binding-center">
    <Card className="border-amber-300 bg-amber-50/60 dark:border-amber-800 dark:bg-amber-950/20">
      <CardHeader>
        ...
        <CardDescription>
          Retell, Telnyx, and GoHighLevel are treated as tenant-bound evidence—not browser configuration forms.
        </CardDescription>
      </CardHeader>
        ...
        <div className="flex items-start gap-3 rounded-md border border-amber-300 bg-background/80 p-3 dark:border-amber-800">
          ...
          <div>
            ...
            <p className="mt-1 text-muted-foreground">
              There is deliberately no API-key field, activate switch, number import, connection test, or spend action here. A green UI control would not prove the account, phone number, webhook, or balance belongs to the selected tenant.
            </p>
          </div>
        </div>
      ...
      {PROVIDER_READINESS.map((provider) => (
        <Card key={provider.name}>
          <CardHeader className="pb-3">
            <div className="flex items-start justify-between gap-3"><div><CardTitle className="text-base">{provider.name}</CardTitle><CardDescription className="mt-1">{provider.focus}</CardDescription></div><Badge variant="secondary">Evidence required</Badge></div>
          </CardHeader>
          <CardContent>
