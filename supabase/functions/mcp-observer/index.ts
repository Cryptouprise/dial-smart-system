/**
 * MCP R0 observer adapter.
 *
 * This edge entry point is deliberately hard-locked. Activation requires a
 * reviewed code change after an MCP-specific, revocable API key is bound to
 * exactly one active installation/principal/organization and durable receipt
 * evidence is deployed. No environment variable can unlock it.
 */
...
import {
  ...
  mcpObserverDisabledResponse,
} from "./handler.ts";

const MCP_OBSERVER_LAUNCH_CERTIFIED: boolean = false;
...
serve((request) => {
  if (!MCP_OBSERVER_LAUNCH_CERTIFIED) {
    return mcpObserverDisabledResponse();
  }

  return handleMcpObserverRequest(request, {
    enabled: true,
    resolveServerIdentity: (credential) =>
      observerRuntime().resolveMcpIdentity(credential),
    submitObserverCommand: (submission) =>
      observerRuntime().submitMcpCommand(submission),
  });
});
