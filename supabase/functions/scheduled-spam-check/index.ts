...
function isScheduledSpamCheckCertified(): boolean {
  return false;
}
...
serve(async (req) => {
  ...
  // Launch containment: this scheduler can scan and release phone numbers for
  // every tenant. Restore only behind an exact cron token and a certified,
  // tenant-aware spam-analysis boundary.
  if (!isScheduledSpamCheckCertified()) {
    return new Response(JSON.stringify({
      success: false,
      disabled: true,
      error_code: 'SCHEDULED_SPAM_CHECK_NOT_CERTIFIED',
      error: 'Scheduled spam checks are disabled until exact scheduler authorization is certified.',
    }), {
      status: 503,
