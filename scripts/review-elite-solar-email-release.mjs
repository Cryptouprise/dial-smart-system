...
function usage() {
  return 'Usage: node scripts/review-elite-solar-email-release.mjs --draft <external-draft.json> --handoff <external-handoff.json> --release <external-release.json> --hmac-key-file <external-key.bin>';
}
...
function parseArguments(argv) {
  const values = {};
  const options = new Set(['--draft', '--handoff', '--release', '--hmac-key-file']);
  ...
  if (Object.keys(values).length !== options.size) throw new Error(usage());
  return values;
}
