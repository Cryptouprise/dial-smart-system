...
const SURFACES = Object.freeze([
  ...
  },
  {
    ...
    state: 'hard_disabled',
    required: ['PROVIDER_ADMIN_NOT_CERTIFIED', 'Twilio provider administration is disabled'],
    mustPrecede: "const supabaseUrl = Deno.env.get('SUPABASE_URL');",
  },
  {
    id: 'telnyx_assistant_administration',
    file: 'supabase/functions/telnyx-ai-assistant/index.ts',
    state: 'hard_disabled',
    disabledFunction: 'function isTelnyxAssistantManagementTenantCertified(): boolean {',
    required: ['TELNYX_ASSISTANT_MANAGEMENT_NOT_TENANT_CERTIFIED'],
    mustPrecede: "const supabaseUrl = Deno.env.get('SUPABASE_URL')!;",
  },
  {
    id: 'retell_maintenance_administration',
    file: 'supabase/functions/retell-force-webhook/index.ts',
    state: 'hard_disabled',
    disabledFunction: 'function isRetellMaintenanceRouteTenantCertified(): boolean {',
    required: ['RETELL_MAINTENANCE_ROUTE_NOT_TENANT_CERTIFIED'],
    mustPrecede: "const supabaseUrl = Deno.env.get('SUPABASE_URL');",
  },
  {
    id: 'demo_retell_provisioning',
    file: 'supabase/functions/setup-lady-jarvis/index.ts',
    state: 'hard_disabled',
    required: ['CONTROL_PLANE_NOT_CERTIFIED', 'Demo provider provisioning is disabled'],
    mustPrecede: "const retellApiKey = Deno.env.get('RETELL_AI_API_KEY');",
  },
  {
