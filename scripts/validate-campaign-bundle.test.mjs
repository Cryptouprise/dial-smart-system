...
test('Solar Exit bundle is structurally ready as a locked offline artifact', () => {
  ...
  assert.equal(bundle.manifest.campaign.campaign_type, 'database_reactivation');
  assert.equal(bundle.reactivation.mode, 'consented_database_reactivation');
  assert.equal(bundle.reactivation.historical_interest_alone_authorizes_contact, false);
...
test('database reactivation never treats historical activity as contact authority', () => {
  const bundle = structuredClone(loadSolarExitBundle());
